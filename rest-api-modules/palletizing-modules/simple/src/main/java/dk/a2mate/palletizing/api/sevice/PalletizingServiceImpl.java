...
import dk.a2mate.palletizing.api.internal.BlockStacking;
...
public class PalletizingServiceImpl implements PalletizingService {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(PalletizingServiceImpl.class);
	...
	@Override
	public PalletizedPallet generatePalletizedPallet(String palletId, String itemId, String pattern, String noOfItems,
			String noOfLayers) {
		LOGGER.debug("generatePalletizedPallet {}{}", palletId, itemId);
		Pallet pallet = palletDataService.findById(palletId);
		Item item = itemDataService.findById(itemId);

		PalletizingPattern p = PalletizingPattern.valueOf(pattern == null ? "BLOCK" : pattern);
		Stacking stacking = PatternFactory.getPattern(p, pallet, item);
		PalletLayer[] pl = stacking.generateStacking();
	...
	/*
	 * 2. Stabilize Boxes It’s a simple physics law: Place the heaviest boxes on the
	 * bottom of the pallet (the top layer should be full for steadiness)., but if
	 * there are not enough boxes to fully stack a pallet, place the last few along
	 * the outside edge.
	 * 
	 * 3. Don’t Neglect Wide Gaps Some wooden pallets have wide gaps between boards
	 * (around 4 inches). When you bridge this gap with a box, you create an
	 * overhang situation which reduces a box’s compression resistance potential.
	 * 
	 * 4. Stack in Columns In order to reduce the danger of damage and for greater
	 * stability, boxes should be stacked in columns with one box directly over the
	 * other. Also, stick items as close to each other as possible. Column stacks
	 * prove to be the best way to make pallets stable and safe for shipment.
	 * 
	 * 5. Don’t Interlock Boxes Interlocking can reduce strength by 50 percent and
	 * lead to freight damage. Since two-third of potential compression strength is
	 * in vertical edges and corners, it is important to stack boxes edge-to-edge
